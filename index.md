麒麟框架是一个超轻量级的“沙盒”，适用于Linux、MacOS、Windows、FreeBSD、DOS、UEFI和MBR。它支持x86（16、32和64位）、arm、arm64和MIPS。然而，麒麟框架不是旨于构建另一个“沙盒”工具，而是为逆向工程设计的框架。因此，二进制指令插入和API是麒麟框架的主要及优先关注点。

API丰富的麒麟框架将逆向工程及二进制代码插桩提升到了一个新的层次。麒麟框架提供对寄存器、内存、文件系统和操作系统级别的API访问。麒麟框架还提供了虚拟机级别的API，如保存和恢复执行状态。

---
<h1>麒麟框架也在各大国际安全峰会亮相</h1>
- 2020年:
    - Black Hat, 美国
    - Black Hat, 亚洲
